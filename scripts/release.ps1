...
.DESCRIPTION
    ...
    3. Runs dotnet publish -> publish\win-x64\ClipboardManager.exe
...
dotnet publish $csproj --configuration Release /p:PublishProfile=win-x64 --nologo
...
if (-not $SkipInstaller) {
    ...
    if (-not $iscc) {
        Write-Host "  [WARN] iscc.exe not found - skipping installer." -ForegroundColor Yellow
        Write-Host "         Install Inno Setup 6: https://jrsoftware.org/isdl.php" -ForegroundColor Yellow
    } else {
